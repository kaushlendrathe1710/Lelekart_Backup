...
async function notifyAdminsAboutOrder(order: any, buyer: User): Promise<void> {
  try {
    console.log(
      `Sending in-app notifications to admin users about order #${order.id}`
    );
    ...
    console.log(
      `Found ${adminUsers.length} admin users to notify about order #${order.id}`
    );
    ...
    console.log(
      `Successfully sent in-app notifications to ${adminUsers.length} admin users about order #${order.id}`
    );
  } catch (error) {
...
export async function processMultiSellerOrder(orderId: number): Promise<void> {
  ...
  try {
    ...
    console.log(
      `Order ${orderId} has items from ${Object.keys(itemsBySeller).length} sellers`
    );
    ...
    for (const sellerId in itemsBySeller) {
      ...
      console.log(
        `Created seller order ${sellerOrder.id} for seller ${sellerId} on main order ${orderId}`
      );
      ...
      console.log(
        `Seller notification created for sellerId ${sellerId} and orderId ${sellerOrder.id}`
      );
      ...
      for (const item of sellerItems) {
        try {
          ...
          console.error(
            `Error updating stock for item ${item.id}:`,
            stockError
          );
    ...
    // Fetch the order to get all discounts and payment method
    ...
    // Apply COD handling fee (₹7) if payment method is COD
    const codHandlingFee = mainOrder?.paymentMethod === "cod" ? 7 : 0;
    const newTotal =
      allSubtotal +
      allDeliveryCharges -
      walletDiscount -
...
async function updateProductStock(
  orderItem: OrderItem & { product: any }
): Promise<void> {
  try {
    ...
    if (variantId) {
      ...
      if (variant.stock !== null && variant.stock !== undefined) {
        ...
        console.log(
          `Updated variant ${variantId} stock from ${variant.stock} to ${newStock}`
        );
    ...
    console.log(
      `Updated product ${orderItem.productId} stock from ${product.stock} to ${newStock}`
    );
  } catch (error) {
...
async function notifySellerAboutOrder(
  ...
  try {
    ...
    console.log(
      `Notification sent to seller ${sellerId} for order ${sellerOrderId}`
    );
  } catch (error) {
    console.error(
      `Error notifying seller ${sellerId} about order ${sellerOrderId}:`,
      error
    );
  }
}
 ...
 */
export async function getSellerOrderWithDetails(
  sellerOrderId: number
): Promise<any> {
  const sellerOrder = await storage.getSellerOrderById(sellerOrderId);
  if (!sellerOrder) {
