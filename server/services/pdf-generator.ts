...
const PDF_OPTIONS: pdf.CreateOptions = {
  ...
  footer: {
    height: '10mm',
    contents: {
      default: '<div style="text-align: center; font-size: 10px;">Generated by LeleKart</div>'
    }
  }
};
 ...
 */
export function generatePdfBuffer(templateType: string, data: any): Promise<Buffer> {
...
export async function generatePdf(res: any, html: string, filename: string): Promise<void> {
  try {
    ...
    console.log('Generating PDF with HTML content (first 500 chars):', html.substring(0, 500));
    
    // Create PDF generation options with proper settings
    const pdfOptions: pdf.CreateOptions = {
      ...PDF_OPTIONS,
      // Force encoding to avoid issues with certain characters
      base: `file://${process.cwd()}/`,
      timeout: 30000, // Increase timeout to allow for larger PDFs
    };
