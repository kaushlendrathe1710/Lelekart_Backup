...
const PDF_OPTIONS: pdf.CreateOptions = {
  ...
  footer: {
    ...
    contents: {
      default:
        '<div style="text-align: center; font-size: 10px;">Generated by LeleKart</div>',
    },
  },
};
...
export async function generatePdf(
  ...
  try {
    ...
    // Create PDF generation options with proper settings
    const pdfOptions: pdf.CreateOptions = {
      ...PDF_OPTIONS,
      // Use absolute URLs for resources
      base:
        process.env.NODE_ENV === "production"
          ? process.env.FRONTEND_URL || "https://lelekart.in" // Use your production URL
          : `file://${process.cwd()}/`,
      // Increase timeout for larger PDFs
      timeout: 60000,
      // Add additional settings for consistent rendering
      renderDelay: 1000, // Wait for resources to load
      // Ensure consistent font rendering
      phantomPath: process.env.PHANTOMJS_PATH || undefined,
      // Add custom headers for resource loading
      httpHeaders: {
        "User-Agent":
          "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36",
      },
      // Ensure consistent page size and margins
      format: "A4",
      orientation: "portrait",
      border: {
        top: "10mm",
      ...
      footer: {
        ...
        contents: {
          default:
            '<div style="text-align: center; font-size: 10px; font-family: Arial, sans-serif;">Generated by LeleKart</div>',
        },
      },
    };

    // Generate PDF from HTML using a Promise to ensure proper handling
      ...
      pdf.create(html, pdfOptions).toBuffer((err, buffer) => {
        if (err) {
          ...
          try {
            ...
            // Send PDF buffer directly (more reliable than streaming)
            res.contentType("application/pdf");
            res.send(buffer);
            resolve();
          } catch (sendError) {
            console.error("Error sending PDF buffer:", sendError);
