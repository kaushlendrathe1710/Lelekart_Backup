...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Flipkart product scraper endpoint
  app.post("/api/seller/products/scrape-flipkart", async (req, res) => {
    // Authenticate user
    if (!req.isAuthenticated()) {
      return res.status(401).json({ error: "Authentication required" });
    }
    ...
    try {
      ...
      // Use axios to fetch the product page
      ...
      // These patterns may need adjustment based on Flipkart's HTML structure
      const nameMatch = html.match(/<span[^>]*itemprop="name"[^>]*>(.*?)<\/span>/i) || 
                        html.match(/<h1[^>]*class="[^"]*"[^>]*>(.*?)<\/h1>/i) ||
                        html.match(/<title>(.*?)(?:\s*-\s*Buy.*?)?<\/title>/i);
                        
      const priceMatch = html.match(/<div[^>]*class="[^"]*\_30jeq3[^"]*"[^>]*>(.*?)<\/div>/i) ||
                         html.match(/<div[^>]*class="[^"]*_16Jk6d[^"]*"[^>]*>(.*?)<\/div>/i);
                         
      const mrpMatch = html.match(/<div[^>]*class="[^"]*\_3I9_wc[^"]*"[^>]*>(.*?)<\/div>/i);
      
      const descriptionMatch = html.match(/<div[^>]*class="[^"]*aMaAEs[^"]*"[^>]*>(.*?)<\/div>/is) ||
                               html.match(/<div[^>]*class="[^"]*_2o-xpa[^"]*"[^>]*>(.*?)<\/div>/is);
      
      // Extract images - find all image URLs in the product gallery section
      const imageRegex = /<img[^>]*src="(https:\/\/rukminim[^"]*\.jpg)"[^>]*>/gi;
      const images = [];
      let imgMatch;
      ...
      const categoryMatch = html.match(/<a[^>]*href="\/[^\/]+\/([^\/]+)\/[^>]*>[^<]*<\/a>/i);
      ...
      const specRows = html.match(/<tr[^>]*class="[^"]*_14cfVK[^"]*"[^>]*>(.*?)<\/tr>/gi) || [];
      const specifications = {};
      
      specRows.forEach(row => {
        const keyMatch = row.match(/<td[^>]*class="[^"]*_1hKmbr[^"]*"[^>]*>(.*?)<\/td>/i);
        const valueMatch = row.match(/<li[^>]*class="[^"]*sNqDog[^"]*"[^>]*>(.*?)<\/li>/i);
        
        if (keyMatch && valueMatch) {
          const key = cleanText(keyMatch[1]);
      ...
      const productData = {
        name: nameMatch ? cleanText(nameMatch[1]) : 'Unknown Product',
        price: priceMatch ? cleanPrice(priceMatch[1]) : 0,
        mrp: mrpMatch ? cleanPrice(mrpMatch[1]) : 0,
        description: descriptionMatch ? cleanText(descriptionMatch[1]) : '',
        category: categoryMatch ? cleanText(categoryMatch[1]) : 'Unknown',
