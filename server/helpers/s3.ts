...
async function testS3Connection() {
  try {
    ...
    console.log(
      `S3 Connection successful. Found ${listBuckets.Buckets?.length || 0} buckets.`
    );

    // Check if our bucket exists
    const bucketExists = listBuckets.Buckets?.some(
      (bucket) => bucket.Name === AWS_BUCKET_NAME
    );
    if (bucketExists) {
      console.log(`Bucket '${AWS_BUCKET_NAME}' exists and is accessible.`);
    } else {
      console.warn(
        `Bucket '${AWS_BUCKET_NAME}' was not found in the list of accessible buckets.`
      );
    }
    ...
    console.error(
      "S3 uploads will likely fail. Please check your AWS credentials and network connection."
    );
    return false;
  }
}
...
export async function uploadFile(
  ...
  console.log(
    `S3 Upload - Starting upload for: ${fileKey}, Content Type: ${contentType}, Buffer Length: ${fileBuffer.length}`
  );
  ...
  try {
    ...
    if (!AWS_ACCESS_KEY || !AWS_SECRET_KEY) {
      console.error(
        "AWS credentials are missing. Please check environment variables."
      );
    ...
    if (!AWS_BUCKET_NAME) {
      console.error(
        "AWS_BUCKET_NAME is missing. Please check environment variables."
      );
...
export async function getPresignedDownloadUrl(
  ...
  try {
    ...
    const uniqueKeys = Object.keys(uniqueKeysObj).map((key) =>
