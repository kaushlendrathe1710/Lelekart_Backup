...
export default function SellerProductsPage() {
  ...
  const updateCategoryMutation = useMutation({
    mutationFn: async ({ productId, category, subcategory }: { productId: number, category: string, subcategory: string }) => {
      console.log("Updating product", productId, "with category:", category, "subcategory:", subcategory);
      ...
      if (subcategory && subcategory !== "_none") {
        ...
        const subcategoriesResponse = await fetch(`/api/subcategories?category=${encodeURIComponent(category)}`);
        ...
        (oldData: any) => {
          ...
          return {
            ...
            products: oldData.products.map((p: any) => 
              p.id === data.id ? { ...p, category: data.category, subcategory: data.subcategory } : p
            )
          };
        }
      );
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async ({ queryKey }) => {
      const [_, sellerId, params] = queryKey as [string, number, { 
        page: number, 
        limit: number, 
        search: string, 
  ...
  useEffect(() => {
    ...
    const processProducts = async () => {
      ...
      const neededSubcategoryIds = productsWithSubcategoryIds.map(p => {
  ...
  const fetchedProducts = enhancedProducts.length > 0 ? enhancedProducts : rawProducts;
  ...
  return (
    <SellerDashboardLayout>
            ...
            <Button 
              ...
              onClick={() => {
                ...
                toast({
                  ...
                  description: "Your product data is being prepared for download...",
                });
                        ...
                        {editingProductId === product.id ? (
                          <div className="space-y-2">
                            ...
                            >
                              <SelectTrigger className="h-8 w-full">
                                <SelectValue placeholder={!editingCategory ? "Select category first" : "Select subcategory"} />
                              </SelectTrigger>
                              <SelectContent>
                                ...
                                {(() => {
                                  ...
                                  if (!currentCategory) {
                                    console.log('No current category selected, cannot filter subcategories');
                                    return [];
                                  }
                                  
                                  // Find category object to get ID
                                  const categoryObj = categoriesData?.find((c: any) => c.name === currentCategory);
                                  
                                  if (!categoryObj) {
                                    console.log(`Category "${currentCategory}" not found in categories list`);
                                    return [];
                                  }
                                  
                                  // Filter subcategories to only those matching this category's ID
                                  const allSubcategories = subcategoriesData || [];
                                  const filteredSubcategories = allSubcategories.filter((subcategory: any) => {
                                    return subcategory.categoryId === categoryObj.id;
                                  });
                                  
                                  // Limited logging to avoid console spam
                                  console.log(`Category "${currentCategory}" (ID: ${categoryObj.id}): Found ${filteredSubcategories.length} matching subcategories`);
                                  ...
                                  return filteredSubcategories.map((subcategory: { id: number, name: string }) => (
                              ...
                              <Button 
                                ...
                                onClick={() => {
                                  ...
                                  updateCategoryMutation.mutate({
                                    ...
                                    subcategory: editingSubcategory === "_none" ? "" : editingSubcategory
                                  });
                                }}
                          ...
                          <div 
                            ...
                            onClick={() => {
                              setEditingProductId(product.id);
                              setEditingCategory(product.category || "");
                              setEditingSubcategory(product.subcategory ? product.subcategory : "_none");
                            }}
                          >
                            {product.subcategory || "-"}
                          </div>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end items-center gap-2">
                          ...
                          <Button 
                            ...
                            asChild
                          >
                            <Link href={product.isDraft ? `/seller/drafts/edit/${product.id}` : `/seller/products/edit/${product.id}`}>
      ...
      <div className="mt-2 border-t pt-4 flex items-center">
        <div className="text-sm text-muted-foreground">
          Showing {Math.min(products.length, itemsPerPage)} of {data?.total || 0} products
        </div>
