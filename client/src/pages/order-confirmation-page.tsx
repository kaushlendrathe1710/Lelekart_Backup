...
import { 
  Check, Home, Package, ShoppingBag, Loader2, 
  User, Heart, ShoppingCart, Truck, CreditCard, 
  Gift, HelpCircle, Search, MapPin, Calendar 
} from "lucide-react";
import { useHeroButtons } from "@/hooks/use-hero-buttons";
import { queryClient } from "@/lib/queryClient";

// Helper component for hero buttons
function HeroButtons({ orderId, setLocation }: { orderId?: string, setLocation: (to: string) => void }) {
  ...
  const getButtonIcon = (iconName: string | undefined, buttonTitle: string) => {
    ...
    switch (iconName.toLowerCase()) {
      ...
      default:
        ...
        if (title.includes('shop') || title.includes('home') || title.includes('store')) 
          return <Home className={iconClass} />;
