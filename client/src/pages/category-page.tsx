...
export default function CategoryPage() {
  ...
  if (location.startsWith('/category/')) {
    categoryName = decodeURIComponent(urlParts[urlParts.length - 1].split('?')[0]);
  } else {
  ...
  const category = categories?.find((cat: any) => cat.slug === categoryName) || 
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['/api/products', displayName, currentPage, itemsPerPage, selectedSubcategory],
    queryFn: async () => {
      if (!displayName) return { products: [], pagination: { total: 0, totalPages: 0 } };
      
      let url = `/api/products?category=${encodeURIComponent(displayName)}&page=${currentPage}&limit=${itemsPerPage}`;
    ...
    <div className="container mx-auto py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold">{displayName}{selectedSubcategory ? ` - ${selectedSubcategory}` : ''}</h1>
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              <Button 
                ...
                onClick={() => {
                  ...
                  const newUrl = `/category/${categoryName}${params.toString() ? `?${params.toString()}` : ''}`;
                  setLocation(newUrl);
                }}
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12">
            ...
            {(selectedBrands.length > 0 || priceRange[0] > 0 || priceRange[1] < 100000 || selectedSubcategory) && (
              <Button 
                variant="outline" 
                className="mt-4"
                onClick={handleClearFilters}
