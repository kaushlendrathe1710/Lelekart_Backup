...
export default function ProductViewPage() {
  ...
  const { data: product, isLoading: isProductLoading } = useQuery<Product & { variants?: ProductVariant[] }>({
  ...
  const addToCartMutation = useMutation({
    mutationFn: async (data: { productId: number, quantity: number, variantId?: number }) => {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      toast({
        title: "Added to cart",
        description: `${product.name} ${selectedVariant ? `(${selectedVariant.color} ${selectedVariant.size})` : ""} has been added to your cart`,
        variant: "default",
      });
    } catch (error) {
  ...
  const handleBuyNow = async () => {
    ...
    if (product.variants && product.variants.length > 0 && !isValidSelection) {
      toast({
        title: "Selection Required",
        description: "Please select all required options before proceeding to checkout",
        variant: "destructive",
      });
      return;
    }
          ...
          <div className="bg-white p-6 rounded shadow-sm">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="flex justify-center items-start">
                <ProductImageGallery 
                  imageUrl={selectedVariant?.images ? JSON.parse(selectedVariant.images as string)[0] || product.imageUrl : product.imageUrl}
                  additionalImages={selectedVariant?.images && selectedVariant.images !== "[]" ? selectedVariant.images : product.images}
              ...
              <div>
                  ...
                  {selectedVariant ? (
                    ...
                    selectedVariant.gstDetails ? (
                      <>
                        ...
                        <span className="text-sm text-gray-500 line-through ml-2">
                          ₹{(selectedVariant.gstDetails.priceWithGst * 1.2).toLocaleString('en-IN')}
                        </span>
                      ...
                      <>
                        <span className="text-3xl font-semibold text-gray-900">{formatPrice(selectedVariant.price)}</span>
                        <span className="text-sm text-gray-500 line-through ml-2">₹{(selectedVariant.price * 1.2).toLocaleString('en-IN')}</span>
                        <span className="text-sm text-green-600 ml-2">20% off</span>
                      </>
                    )
                  ) : (
                    ...
                    product.gstDetails ? (
                      <>
                        ...
                        <span className="text-sm text-gray-500 line-through ml-2">
                          ₹{(product.gstDetails.priceWithGst * 1.2).toLocaleString('en-IN')}
                        </span>
                        <span className="text-sm text-green-600 ml-2">20% off</span>
                        <div className="text-sm text-gray-500 mt-1">
                ...
                <div className="mt-6 flex flex-col sm:flex-row gap-4">
                  ...
                  >
                    ...
                    {!isValidSelection && product.variants && product.variants.length > 0 ? 'SELECT OPTIONS' : 'ADD TO CART'}
                  </Button>
                  <Button 
                  ...
                  >
                    ...
                    {!isValidSelection && product.variants && product.variants.length > 0 ? 'SELECT OPTIONS' : 'BUY NOW'}
                  </Button>
                </div>
              </div>
            </div>
