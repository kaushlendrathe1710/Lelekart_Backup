...
import { FileEdit, Trash2, Plus, RefreshCw, Award, Users, ShoppingCart } from 'lucide-react';
...
function RewardsManagement() {
  ...
  const { data: transactionsData, isLoading: isLoadingTransactions } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiRequest('GET', '/api/admin/rewards/transactions?limit=20');
      const data = await response.json();
      return data;
    }
  });
  ...
  const createRuleMutation = useMutation({
    mutationFn: async (ruleData: Partial<RewardRule>) => {
      const response = await apiRequest('POST', '/api/admin/rewards/rules', ruleData);
  ...
  const updateRuleMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: Partial<RewardRule> }) => {
      const response = await apiRequest('PUT', `/api/admin/rewards/rules/${id}`, data);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/admin/rewards/rules'] });
  ...
  const deleteRuleMutation = useMutation({
    mutationFn: async (id: number) => {
      const response = await apiRequest('DELETE', `/api/admin/rewards/rules/${id}`);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/admin/rewards/rules'] });
  ...
  const addPointsMutation = useMutation({
    mutationFn: async (pointsData: typeof pointsToAdd) => {
      const response = await apiRequest('POST', '/api/admin/rewards/points', pointsData);
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/admin/rewards/statistics'] });
      queryClient.invalidateQueries({ queryKey: ['/api/admin/rewards/transactions'] });
      toast({
        title: 'Success',
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-3xl font-bold">
                {isLoadingStats ? '...' : statistics?.totalPointsIssued.toLocaleString()}
              </p>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="pb-2">
              ...
              <CardDescription>Lifetime points redeemed by users</CardDescription>
            </CardHeader>
            <CardContent>
              <p className="text-3xl font-bold">
                {isLoadingStats ? '...' : statistics?.totalPointsRedeemed.toLocaleString()}
              </p>
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent>
              <p className="text-3xl font-bold">
                {isLoadingStats ? '...' : statistics?.activeUsers.toLocaleString()}
              </p>
            </CardContent>
          </Card>
        </div>
                      ...
                      <TableBody>
                        {rules.map((rule: RewardRule) => (
                          <TableRow key={rule.id}>
                            ...
                            <TableCell>
                              <Badge variant="outline" className={`${getTransactionTypeColor(rule.type)}`}>
                                {rule.type}
                              </Badge>
                            </TableCell>
                            ...
                            <TableCell>
                              {rule.minimumOrderValue ? (
                                <div className="text-sm">Min Order: ₹{rule.minimumOrderValue}</div>
                              ) : null}
                              {rule.percentageValue ? (
                                <div className="text-sm">Percentage: {rule.percentageValue}%</div>
                              ) : null}
                              {rule.validFrom ? (
                                <div className="text-sm">
                                  Valid: {new Date(rule.validFrom).toLocaleDateString()} - 
                ...
                {isLoadingTransactions ? (
                  ...
                  <div className="overflow-x-auto">
                    <Table>
                      ...
                      <TableBody>
                        {transactionsData?.transactions?.map((transaction: RewardTransaction) => (
                          <TableRow key={transaction.id}>
                            ...
                            <TableCell>
                              <Badge variant={transaction.status === 'active' ? 'success' : transaction.status === 'used' ? 'secondary' : 'destructive'}>
                                {transaction.status}
                              </Badge>
                            </TableCell>
                          </TableRow>
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex gap-4 mb-6">
                  <div className="flex-1">
                    ...
                    <Input 
                      ...
                      onChange={(e) => setPointsToAdd({...pointsToAdd, userId: Number(e.target.value)})}
                    />
                  </div>
                  <div className="flex-1">
                ...
                <p className="text-muted-foreground text-sm mb-4">
                  To view a specific user's reward transactions, navigate to the Users page and select the user.
                </p>
              </CardContent>
            </Card>
          </TabsContent>
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="grid gap-2">
                ...
                <Input 
                  ...
                  onChange={(e) => setNewRule({...newRule, pointsAwarded: Number(e.target.value)})}
                />
              </div>
                ...
                <>
                  <div className="grid gap-2">
                    ...
                    <Input 
                      ...
                      onChange={(e) => setNewRule({...newRule, minimumOrderValue: Number(e.target.value)})}
                    />
                  </div>
                  <div className="grid gap-2">
                    ...
                    <Input 
                      ...
                      onChange={(e) => setNewRule({...newRule, percentageValue: Number(e.target.value)})}
                    />
                    <p className="text-xs text-muted-foreground">Leave blank to use fixed points instead</p>
                  </div>
                </>
              )}
              ...
              <div className="grid gap-4 py-4">
                ...
                <div className="grid gap-2">
                  ...
                  <Input 
                    ...
                    onChange={(e) => setSelectedRule({...selectedRule, pointsAwarded: Number(e.target.value)})}
                  />
                </div>
                  ...
                  <>
                    <div className="grid gap-2">
                      ...
                      <Input 
                        ...
                        onChange={(e) => setSelectedRule({...selectedRule, minimumOrderValue: Number(e.target.value)})}
                      />
                    </div>
                    <div className="grid gap-2">
                      <Label htmlFor="edit-percentageValue">Percentage Value (%)</Label>
                      <Input 
                        id="edit-percentageValue" 
                        type="number" 
                        placeholder="Enter percentage value" 
                        value={selectedRule.percentageValue || ''} 
                        onChange={(e) => setSelectedRule({...selectedRule, percentageValue: Number(e.target.value)})}
                      />
                      <p className="text-xs text-muted-foreground">Leave blank to use fixed points instead</p>
                    </div>
                  </>
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="grid gap-2">
                ...
                <Input 
                  ...
                  onChange={(e) => setPointsToAdd({...pointsToAdd, description: e.target.value})}
                />
              </div>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setIsAddPointsDialogOpen(false)}>Cancel</Button>
              <Button onClick={handleAddPoints}>Add Points</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
