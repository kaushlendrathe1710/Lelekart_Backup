...
const ShippingDashboardPage = () => {
  ...
  const { 
    ...
    queryFn: async () => {
      try {
        const res = await apiRequest('GET', `/api/shipping/stats?timeframe=${statsTimeframe}`);
        if (!res.ok) {
  ...
  } = useQuery<{orders: PendingOrder[], total: number, page: number, pageSize: number}>({
  ...
  const createShipmentMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['/api/shipping/pending-orders'] });
            ...
            <div className="flex">
              ...
              <div>
                <p className="text-sm text-yellow-700">
                  Shipping is not configured. Please set up your shipping credentials in 
                  <Button 
                    variant="link" 
                    className="px-1 font-medium underline text-yellow-700"
                    onClick={() => window.location.href = '/admin/shipping-settings'}
                  >
                    Shipping Settings
                  </Button>
                </p>
