...
/**
 * Download bulk order invoice PDF
 * Distributors can download their own approved orders
 * Admins can download any approved order
 */
export async function downloadBulkOrderInvoice(orderId: number): Promise<void> {
  const response = await axios.get(`/api/bulk-orders/${orderId}/invoice`, {
    responseType: "blob",
  });
  ...
  link.href = url;
  link.setAttribute("download", `invoice-BO-${orderId}.pdf`);
