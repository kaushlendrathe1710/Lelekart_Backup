...
export function SearchSuggestions({ 
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      console.log("SUGGESTION DEBUG - Starting fetch for suggestions with query:", debouncedQuery);
      ...
      try {
        ...
        console.log("SUGGESTION DEBUG - Calling AISearchService.getSuggestions with query:", currentQuery);
        
        const results = await AISearchService.getSuggestions(currentQuery, isAdmin);
        
        // Only update state if the query is still the same (prevent stale results)
        if (currentQuery === debouncedQuery.trim()) {
          console.log(`SUGGESTION DEBUG - Got ${results.length} suggestions for "${currentQuery}"`);
          setSuggestions(results);
          setError(null);
        } else {
          console.log("SUGGESTION DEBUG - Query changed during fetch, discarding results");
        }
      } catch (err) {
            ...
            <li key={suggestion.id}>
              ...
              >
                ...
                <div className="flex-1 min-w-0 text-left">
                  ...
                  <p className="text-xs text-gray-500 truncate">in {suggestion.category}</p>
                </div>
