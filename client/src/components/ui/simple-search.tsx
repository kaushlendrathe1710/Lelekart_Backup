...
export function SimpleSearch({
  ...
  const processAiSearch = async (searchQuery: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const searchUrl = AISearchService.buildSearchUrl(
          result.filters,
  ...
  const addToBrowserHistory = (searchTerm: string) => {
    ...
    try {
      ...
      history = history.filter(
        (item: string) => item.toLowerCase() !== searchTerm.trim().toLowerCase()
      );
      // Add to start
  ...
  const handleSuggestionSelect = (suggestion: any) => {
    ...
    // Prevent the click from bubbling up to avoid triggering useOnClickOutside
    // Navigate first, then clear the search
    if (variant === "admin") {
      // Admin path navigation - use Wouter navigation
      console.log(
        `Navigating to admin product: /admin/products/${suggestion.id}`
      );
    ...
    // Clear query and close suggestions after navigation
    setTimeout(() => {
      setQuery("");
      setShowSuggestions(false);
    }, 100);
