    ...
    try {
      ...
      razorpay.on('payment.failed', function (response: any) {
        ...
        if (response.error && 
            ...
            response.error.description.includes('This business is not allowed to accept payments on this website')) {
          
          console.log('Detected unregistered website error, attempting to continue anyway');
        ...
        try {
          ...
          if (errorObj) {
            if (errorObj.error && errorObj.error.code === 'BAD_REQUEST_ERROR' && 
                errorObj.error.description === 'This business is not allowed to accept payments on this website. We suggest not going ahead with the payment.') {
              
              // This is the "unregistered website" error
              console.log('Detected unregistered website error from API - this is expected in development');
              
              errorMessage = 'Development environment: Domain not registered with Razorpay. In production, add this domain to your Razorpay account dashboard.';
              
              // Display a more helpful error message
              toast({
                title: 'Razorpay Domain Verification Required',
                description: 'For production use, register your domain in the Razorpay dashboard under Settings > Websites & Apps.',
                variant: 'default',
              });
          ...
          } else if (error.message.includes('Authentication failed') || error.message.includes('invalid auth')) {
    ...
    <Card className="w-full">
      ...
      <CardContent>
        ...
        <div className="mt-3 p-3 border border-blue-200 bg-blue-50 rounded-md">
          <p className="text-sm text-blue-800 font-medium">Development Environment</p>
          <p className="text-xs text-blue-700 mt-1">
            This is a development environment. In production, you'll need to register your domain in the Razorpay dashboard 
            under Settings &gt; Websites &amp; Apps to avoid domain verification errors.
          </p>
        </div>
      </CardContent>
      <CardFooter>
