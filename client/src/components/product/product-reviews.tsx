...
const ProductReviews: React.FC<ProductReviewsProps> = ({ productId }) => {
  ...
  const {
    data: userHelpfulStatuses,
    isLoading: isHelpfulStatusesLoading,
        ...
        await Promise.all(
          reviews.map(async (review) => {
            try {
              ...
              console.error(`Error checking helpful status for review ${review.id}:`, error);
            }
          })
        );
      }
  ...
  const userHasReviewed = reviews?.some((review) => review.userId === user?.id) || false;
  ...
  const verifiedReviews = reviewsWithHelpfulStatus.filter((review) => review.verifiedPurchase);
