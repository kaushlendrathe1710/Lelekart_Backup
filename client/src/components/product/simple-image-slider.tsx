...
export default function SimpleImageSlider({
  ...
  const extractedProductImages = Array.isArray(images)
    ...
    : [];
  const extractedVariantImages = Array.isArray(selectedVariantImages)
    ? extractImageUrls(selectedVariantImages)
    : [];
  ...
  if (selectedVariant && extractedVariantImages.length > 0) {
    ...
    console.log(
      "Using only variant images:",
      extractedVariantImages.length,
      "images"
    );
  } else {
    ...
    console.log(
      "Using only product images:",
      extractedProductImages.length,
      "images"
    );
  }
