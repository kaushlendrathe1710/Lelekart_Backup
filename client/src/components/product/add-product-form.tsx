...
import { calculateGstAmount, calculatePriceWithGst, formatPriceWithGstBreakdown } from "@shared/utils/gst";
import { 
  ImagePlus, 
  Tag, 
  AlertCircle,
...
export default function AddProductForm({ 
  ...
  const categoryGstRates = categoriesData?.reduce((acc: Record<string, number>, category: any) => {
  ...
  const watchedFields = form.watch(["name", "category", "price", "description", "stock", "gstRate"]);
  ...
  const calculateFinalPrice = (): { basePrice: number; gstRate: number; gstAmount: number; finalPrice: number } => {
    ...
    const gstRate = customGstRate !== undefined && customGstRate !== null ? parseFloat(customGstRate.toString()) : getSelectedCategoryGstRate();
  ...
  const handleAddImage = useCallback((fileOrUrl: File | string) => {
    ...
    if (typeof fileOrUrl === "string") {
      ...
      try {
        ...
        if (!hasValidExtension) {
          ...
          toast({
            title: "URL added",
            description: "The URL doesn't end with a common image extension. Please verify the image appears correctly.",
            variant: "default",
          });
        } else {
          toast({
    ...
    try {
      ...
      if (isFirefoxBrowser) {
        console.log("Detected Firefox browser, using safe FileReader implementation for main image");
      }
      ...
      reader.onload = (event) => {
        if (event.target && event.target.result) {
          ...
          setUploadedImages(prevImages => [...prevImages, event.target.result as string]);
        }
      };
      reader.onerror = () => {
  ...
  const saveDraftMutation = useMutation({
    mutationFn: async (data: any) => {
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json().catch(() => null);
          if (errorData && errorData.error) {
            throw new Error(typeof errorData.error === 'string' ? errorData.error : "Failed to save draft");
          }
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: "Your product draft has been saved and can be edited later.",
      });
      // Don't navigate immediately to let seller see the updated list
    },
    onError: (error: Error) => {
  ...
  const handleAddVariant = () => {
    ...
    const safePrice = parseFloat(form.getValues("price")?.toString() || "0") || 0;
    ...
    const newVariant: ProductVariant = {
      ...
      sku: formSku ? `${formSku}-${variantsCount + 1}` : `SKU-${uniqueId}-${variantsCount + 1}`,
      color: "",
      size: "",
      price: safePrice,
      mrp: safeMrp,
  ...
  const handleSaveNewVariant = () => {
    if (currentVariant) {
      ...
      if (!currentVariant.sku || !currentVariant.price || currentVariant.stock === undefined) {
        toast({
      ...
      setDraftVariants(prevDraftVariants => {
        ...
        const variantExists = prevDraftVariants.some(v => v.id === variantWithImages.id);
        if (variantExists) {
  ...
  const handleUpdateCurrentVariant = (field: keyof ProductVariant, value: any) => {
    if (currentVariant) {
      setCurrentVariant({
        ...currentVariant,
        [field]: value
  ...
  const handleAddVariantImage = (fileOrUrl: File | string) => {
    ...
    try {
      ...
      reader.onload = (event) => {
        if (event.target && event.target.result) {
          // Use functional state update to prevent race conditions
          setVariantImages(prevImages => [...prevImages, event.target.result as string]);
        }
      };
      reader.readAsDataURL(fileOrUrl);
    } catch (error) {
  ...
  const getFieldValue = (field: keyof ProductVariant) => {
    return currentVariant && currentVariant[field] !== undefined && currentVariant[field] !== null
  ...
  const updateProductMutation = useMutation({
    mutationFn: async (data: any) => {
      try {
        ...
        const response = await apiRequest("PUT", `/api/products/${initialValues.id}`, data);
        ...
        if (!response.ok) {
          ...
          throw new Error("Failed to update product. Please check all required fields.");
        }
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (formValues.price === undefined || formValues.price === null || isNaN(Number(formValues.price))) {
        toast({
          title: "Invalid Price",
      ...
      const processedFormValues = {
        ...
        purchasePrice: formValues.purchasePrice ? Number(formValues.purchasePrice) : formValues.purchasePrice,
        gstRate: formValues.gstRate ? Number(formValues.gstRate) : formValues.gstRate,
      };
  ...
  const isCustomRate = form.watch("gstRate") !== undefined && form.watch("gstRate") !== null;
  
  // Calculate GST amount and final price
  const gstRate = isCustomRate ? form.watch("gstRate") || 0 : getSelectedCategoryGstRate();
  ...
  const isSubmitting = createProductMutation.isPending || updateProductMutation.isPending || saveDraftMutation.isPending;
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      ...
      if (inputValue.trim()) {
        ...
        const fieldName = (e.currentTarget.closest('[data-field]') as HTMLElement)?.dataset?.field;
            ...
            <CardTitle className="text-xl flex items-center">
              <CheckCircle 
                className={`mr-2 h-5 w-5 ${
                  completionStatus.percentage === 100 ? 'text-green-500' : 'text-muted-foreground'
                }`} 
              />
              Completion Status
            </CardTitle>
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 pt-2">
              <div className={`flex items-center gap-2 rounded-md border p-2 ${
                completionStatus.basicComplete ? 'border-green-500 bg-green-50 dark:bg-green-950/20' : 'border-gray-200'
              }`}>
                <span className={`rounded-full p-1 ${
                  completionStatus.basicComplete ? 'bg-green-100 text-green-600 dark:bg-green-900 dark:text-green-400' : 'bg-gray-100 text-gray-400'
                }`}>
              ...
              <div className={`flex items-center gap-2 rounded-md border p-2 ${
                completionStatus.descriptionComplete ? 'border-green-500 bg-green-50 dark:bg-green-950/20' : 'border-gray-200'
              }`}>
                <span className={`rounded-full p-1 ${
                  completionStatus.descriptionComplete ? 'bg-green-100 text-green-600 dark:bg-green-900 dark:text-green-400' : 'bg-gray-100 text-gray-400'
                }`}>
              ...
              <div className={`flex items-center gap-2 rounded-md border p-2 ${
                completionStatus.inventoryComplete ? 'border-green-500 bg-green-50 dark:bg-green-950/20' : 'border-gray-200'
              }`}>
                <span className={`rounded-full p-1 ${
                  completionStatus.inventoryComplete ? 'bg-green-100 text-green-600 dark:bg-green-900 dark:text-green-400' : 'bg-gray-100 text-gray-400'
                }`}>
              ...
              <div className={`flex items-center gap-2 rounded-md border p-2 ${
                completionStatus.imagesComplete ? 'border-green-500 bg-green-50 dark:bg-green-950/20' : 'border-gray-200'
              }`}>
                <span className={`rounded-full p-1 ${
                  completionStatus.imagesComplete ? 'bg-green-100 text-green-600 dark:bg-green-900 dark:text-green-400' : 'bg-gray-100 text-gray-400'
                }`}>
              ...
              <CardContent className="space-y-4">
                ...
                <FormField
                  ...
                  render={({ field }) => {
                    ...
                    const categoryObject = categoriesData?.find((c: any) => c.name === selectedCategory);
                    
                    // Filter subcategories by the selected category
                    const filteredSubcategories = subcategoriesData?.filter((sc: any) => {
                          ...
                          <Select 
                            onValueChange={(value) => {
                              ...
                              if (value === "") {
                                ...
                                const subcategory = subcategoriesData?.find((sc: any) => sc.name === value);
                                if (subcategory) {
                  ...
                  <Card className="bg-muted/30">
                    <CardContent className="p-4 space-y-2">
                      <div className="flex items-center">
                        ...
                        <span className="text-sm font-medium">GST Information</span>
                      </div>
                      <div className="grid grid-cols-1 gap-2 text-sm">
                        {watchedFields.category && (
                          <>
                            <p className="text-slate-600">
                              <span className="font-medium">Category:</span> {watchedFields.category}
                            </p>
                            <p className="text-slate-600">
                            ...
                            {watchedFields.price && (
                              <div className="pt-1 space-y-1">
                                <p className="text-slate-600">
                                  <span className="font-medium">Base Price:</span> ₹{(typeof basePrice === 'number' ? basePrice.toFixed(2) : '0.00')}
                                </p>
                                <p className="text-slate-600">
                                  <span className="font-medium">GST Amount:</span> ₹{(typeof gstAmount === 'number' ? gstAmount.toFixed(2) : '0.00')} ({gstRate}%)
                                </p>
                                <p className="text-slate-600 font-medium">
                                  <span className="font-medium">Final Price:</span> ₹{(typeof finalPrice === 'number' ? finalPrice.toFixed(2) : '0.00')}
                                </p>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="pt-2">
                  ...
                  <div className="space-y-4">
                    ...
                    {/* Variant Table */}
                    {(variants.length > 0 || draftVariants.length > 0 || newVariantRow) && (
                              ...
                              <TableRow>
                                <TableCell>
                                  <Input
                                    ...
                                    onChange={(e) => handleUpdateCurrentVariant('sku', e.target.value)}
                                ...
                                <TableCell>
                                  <Input
                                    ...
                                    onChange={(e) => handleUpdateCurrentVariant('color', e.target.value)}
                                    className="h-8 w-full"
                                    placeholder="Color"
                                  />
                                </TableCell>
                                <TableCell>
                                  <Input
                                    ...
                                    onChange={(e) => handleUpdateCurrentVariant('size', e.target.value)}
                                    className="h-8 w-full"
                                    placeholder="Size"
                                  />
                                </TableCell>
                                <TableCell>
                                  <Input
                                    ...
                                    onChange={(e) => handleUpdateCurrentVariant('price', parseFloat(e.target.value) || 0)}
                                    className="h-8 w-full"
                                    placeholder="Price"
                                  />
                                </TableCell>
                                <TableCell>
                                  <Input
                                    ...
                                    onChange={(e) => handleUpdateCurrentVariant('stock', parseInt(e.target.value) || 0)}
                                    className="h-8 w-full"
                                    placeholder="Stock"
                                  />
                                </TableCell>
                    ...
                    <p className="text-xs text-muted-foreground mt-2">
                      Use variants to define different combinations of properties like color and size, each with its own price, SKU, and stock level.
                    </p>
                  </div>
                </div>
              </CardContent>
          ...
          <div className="space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <p className="text-xs text-muted-foreground mt-2">
                  First image will be used as the main product image. You can upload up to 8 images (max 5MB each).
                </p>
              </CardContent>
            </Card>
                ...
                <div className="border rounded-md divide-y">
                  <div className="p-3 flex items-start gap-3 bg-muted/30">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        All products are subject to approval before being listed in the marketplace. 
                  ...
                  <div className="p-3 flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        If you have any questions about adding products, please contact our seller support team.
                      </p>
                    </div>
                  </div>
                </div>
