...
export function Footer() {
  ...
  const groupedContent = footerContents.reduce((acc: Record<string, FooterContent[]>, item: FooterContent) => {
    if (item.isActive) {
      if (!acc[item.section]) {
        acc[item.section] = [];
      }
  ...
  Object.keys(groupedContent).forEach(section => {
    groupedContent[section].sort((a, b) => a.order - b.order);
  });
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm">
                {groupedContent.about.map((item) => (
                  <li key={item.id}>
                    <Link href="#" className="hover:underline" onClick={() => console.log(`Clicked: ${item.title}`)}>
                      {item.title}
                    </Link>
                  </li>
                ))}
          ...
          {groupedContent.consumer_policy && (
            <div>
              ...
              <ul className="space-y-2 text-sm">
                {groupedContent.consumer_policy.map((item) => (
                  <li key={item.id}>
                    <Link href="#" className="hover:underline" onClick={() => console.log(`Clicked: ${item.title}`)}>
                      {item.title}
                    </Link>
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm">
                {groupedContent.social.map((item) => (
                  <li key={item.id}>
                    <Link href="#" className="hover:underline flex items-center" onClick={() => console.log(`Clicked: ${item.title}`)}>
