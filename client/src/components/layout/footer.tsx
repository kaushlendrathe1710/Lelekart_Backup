...
const formatPath = (path: string): string => {
  ...
  if (trimmedPath.toLowerCase() === 'about-us' || 
...
export function Footer() {
  ...
  Object.keys(groupedContent).forEach(section => {
    groupedContent[section].sort((a: FooterContent, b: FooterContent) => a.order - b.order);
  });
              ...
              {groupedContent.mail_us && groupedContent.mail_us.length > 0 ? (
                <div className="mt-6">
                  ...
                  <p className="text-xs leading-relaxed">
                    {groupedContent.mail_us.length === 1 && groupedContent.mail_us[0].content.includes('\n') ? (
                      // Handle case where content has embedded newlines
                      groupedContent.mail_us[0].content.split('\n').map((line: string, index: number) => (
                        <span key={index}>
                          {line}
                          {index < groupedContent.mail_us[0].content.split('\n').length - 1 && <br />}
                        </span>
                      ))
                    ) : (
                      ...
                      groupedContent.mail_us
                        ...
                        .map((item: FooterContent, index: number) => (
                          <span key={item.id}>
                            {item.content}
                            {index < groupedContent.mail_us.length - 1 && <br />}
                          </span>
                        ))
                    )}
                  </p>
